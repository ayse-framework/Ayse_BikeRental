    ...
    for id, bike in pairs(Config.cycles) do
        bikes[#bikes + 1] = {
            ...
            onSelect = function(args)
                selectedBike = id
                TriggerServerEvent("Ayse_BikeRental:pay", id)
            end,
            metadata = {
...
RegisterNetEvent("Ayse_BikeRental:confirm")
AddEventHandler("Ayse_BikeRental:confirm", function()
...
RegisterNetEvent("Ayse_BikeRental:deny")
AddEventHandler("Ayse_BikeRental:deny", function()
    if not selectedBike then return end
    local bike = Config.cycles[selectedBike]
    selectedBike = nil 
    lib.notify({
